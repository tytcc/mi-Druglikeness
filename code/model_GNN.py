...
import numpy as np
from chemprop.features.featurization import BatchMolGraph,mol2graph,MolGraph
...
from chemprop_evaluate import predict
...
from chemprop.utils import build_lr_scheduler, load_checkpoint,\
...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        """
        if type(mol_graph[0]) != BatchMolGraph:
            mol_graph = mol2graph(mol_graph[0])
            # BatchMolGraph([MolGraph(mol) for mol in mol_graph])
            # mol_graph=BatchMolGraph(mol_graph)
        else:
...
class train_GNN():
    ...
    def train(self,train_data,val_data,iteration,args,logger):
        ...
        save_checkpoint(os.path.join(save_dir, 'model_iteration_'+str(iteration)+'.pt'), model, scaler, features_scaler, args=args)
        ...
        for epoch in trange(args.epochs):
            ...
            if args.minimize_score and avg_val_score < best_score or \
                ...
                save_checkpoint(os.path.join(save_dir, 'model.pt'), model, scaler, features_scaler, args=args)
